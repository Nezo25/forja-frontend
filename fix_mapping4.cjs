...
c = c.replace(/fetchApi\('\/orders'\)[\s\S]*?\.catch\(console\.error\);/, etchApi('/orders').then((data: any) => {
...
c = c.replace(/fetchApi\('\/admin\/filaments'\)[\s\S]*?\.catch\(console\.error\);/, etchApi('/admin/filaments').then((data: any) => {
      if (Array.isArray(data)) {
